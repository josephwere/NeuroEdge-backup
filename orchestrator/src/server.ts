...
wss.on("connection", (ws: WebSocket) => {
  ws.on("message", async (message) => {
    try {
      ...
      const kernelId: string = payload.kernelId || "local";
...
app.post("/kernels", (req: Request, res: Response) => {
  ...
  if (!id || !baseUrl) {
    return res.status(400).json({ error: "id and baseUrl required" });
  }
...
async function validateWithKernel(
  kernelId: string,
  _proposal: { explanation: string }
